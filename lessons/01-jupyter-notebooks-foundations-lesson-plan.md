...
| - Describe what a Jupyter Notebook is and why it is useful for analysis.<br>- Distinguish markdown cells from code cells.<br>- Run cells in order and explain `Shift+Enter` workflow.<br>- Import libraries in a notebook and explain why imports are usually near the top.<br>- Use simple output cells to confirm code is working. |
| :---- |
...
| 1. Opening comparison: students compare a plain script view to a notebook view and discuss which is easier for learning.<br>2. Notebook setup: students open Jupyter or Colab and create a new notebook.<br>3. Cell practice: students add one markdown cell and one code cell, then run them in order.<br>4. Vocabulary mini-lesson: teacher explains import, library, DataFrame, method, property, kernel/runtime, and execution order using the companion tutorial.<br>5. Guided mini-sequence: students import libraries, create a tiny DataFrame, preview rows, inspect columns, and make a simple chart.<br>6. Troubleshooting practice: students restart the kernel/runtime and rerun all cells from top to bottom.<br>7. Reflection/checkpoint: students explain one execution-order mistake and how to fix it. |
| :---- |

## Lesson Body

| This lesson teaches notebook mechanics and workflow so students can focus on analysis in Lesson 02. |
| :---- |

### Engage

| Show two screenshots: plain script vs notebook with text + code + output. Ask which format is easier for learning and why. |
| :---- |

### Explore

| Students create a notebook, add one markdown cell and one code cell, then run cells in order. They practise restarting kernel/runtime and re-running top-to-bottom. |
| :---- |

### Explain

| Explain notebook structure (cells), execution order, why imports are grouped at the top, and how outputs reflect the most recently run state. |
| :---- |

### Elaborate

| Students run a short mini-sequence: import libraries, create small DataFrame, inspect `head()`, print column names, and make a tiny chart. |
| :---- |

### Evaluate

| Evidence from observations, conversations, and student products. Checkpoint: student can correctly create markdown/code cells, run all cells in order, and explain one execution-order mistake and fix. |
| :---- |

## Homework

| In 5-7 sentences, explain how Jupyter notebooks help with reproducibility and communication in data analysis. |
| :---- |
